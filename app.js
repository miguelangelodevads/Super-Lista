...
function init() { router('home'); updateCartBadge(); }
...
function router(page) {
    ...
    // Resetar navegação
    document.querySelectorAll('.bottom-nav-item').forEach(el => el.classList.remove('active', 'text-pink-500'));
    document.getElementById(`nav-${page}`).classList.add('active', 'text-pink-500');
...
function renderHome(container) {
    const categories = ['Todos', 'Despensa', 'Frios', 'Hortifruti', 'Limpeza', 'Higiene', 'Bebidas', 'Outros'];
    let html = `
...
function renderHistory(container) {
    if (history.length === 0) {
        container.innerHTML = `
            <div class="text-center p-8 mt-10">
                ...
                <p class="text-gray-500">Nenhuma compra finalizada ainda.</p>
            </div>`;
        return;
    }

    const html = history.map(rec => {
        const date = new Date(rec.date).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
        const itemCount = Object.keys(rec.items).length;
        
        // ADICIONADO: Botão de Lixeira (Excluir) e Botão de Repetir
        return `
        <div class="bg-white p-4 rounded-2xl shadow-sm border-l-4 border-pink-400 mb-4 mx-4 mt-4 relative">
            <div class="flex justify-between items-start mb-2">
                <div>
                    <h3 class="font-bold text-gray-800 capitalize">${date}</h3>
                    <p class="text-sm text-gray-500">${itemCount} itens comprados</p>
                </div>
                <span class="font-bold text-green-600 text-lg">
                    ${rec.total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                </span>
            </div>
            <div class="mt-3 border-t pt-3 flex justify-between items-center">
                 <button onclick="deleteHistoryItem(${rec.id})" class="text-red-400 hover:text-red-600 p-2 rounded-lg transition-colors flex items-center gap-1 font-semibold text-xs border border-red-100 hover:bg-red-50" title="Apagar este registro">
                    <i class="fa-solid fa-trash-can"></i> Excluir
                </button>
                <button onclick="reuseList(${rec.id})" class="text-pink-500 font-bold text-sm flex items-center gap-1 hover:bg-pink-50 px-3 py-1 rounded-lg transition-colors">
                    <i class="fa-solid fa-repeat"></i> Usar novamente
                </button>
            </div>
        </div>
        `;
    ...
    container.innerHTML = `<h2 class="p-4 text-xl font-bold text-gray-700">Minhas Compras</h2>` + html;
}

// NOVA FUNÇÃO: Apagar item do histórico
function deleteHistoryItem(id) {
    if(confirm('🗑️ Quer mesmo apagar este registro do histórico?')) {
...
function renderStats(container) {
    const now = new Date();
    let totalMonth = 0, totalYear = 0, totalAll = 0;
    history.forEach(h => {
...
function filterCategory(cat) { currentFilter = cat; renderHome(document.getElementById('app-content')); }
function toggleCart(id) { if (cart[id]) delete cart[id]; else cart[id] = { qty: 1, checked: false, price: 0 }; saveData(); renderProductGrid(); updateCartBadge(); }
function toggleCheck(id) { cart[id].checked = !cart[id].checked; saveData(); renderCart(document.getElementById('app-content')); }
function changeQty(id, delta) { const newQty = cart[id].qty + delta; if (newQty > 0) { cart[id].qty = newQty; saveData(); renderCart(document.getElementById('app-content')); } }
function updatePrice(id, val) { cart[id].price = parseFloat(val); saveData(); updateTotalHeader(); }
function updateTotalHeader() { let total = 0; Object.values(cart).forEach(item => { if(item.price) total += item.price * item.qty; }); document.getElementById('header-total').innerText = total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
function clearCart() { if(confirm('Tem certeza que quer limpar sua lista?')) { cart = {}; saveData(); updateCartBadge(); router('home'); } }
function finishShopping() { const total = Object.values(cart).reduce((acc, item) => acc + ((item.price || 0) * item.qty), 0); if (Object.keys(cart).length === 0) return; const shopRecord = { id: Date.now(), date: new Date().toISOString(), total: total, items: JSON.parse(JSON.stringify(cart)) }; history.unshift(shopRecord); cart = {}; saveData(); updateCartBadge(); alert(`🎉 Compra finalizada!\nTotal: ${total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`); router('history'); }
function reuseList(histId) { const record = history.find(h => h.id === histId); if (!record) return; if (Object.keys(cart).length > 0 && !confirm('Isso vai substituir sua lista atual. Pode ser?')) return; const newCart = {}; Object.keys(record.items).forEach(key => { newCart[key] = { qty: record.items[key].qty, checked: false, price: 0 }; }); cart = newCart; saveData(); updateCartBadge(); router('cart'); }
function updateCartBadge() { const count = Object.keys(cart).length; const badge = document.getElementById('cart-badge'); badge.innerText = count; if (count > 0) badge.classList.remove('hidden'); else badge.classList.add('hidden'); }
function saveData() { localStorage.setItem('products', JSON.stringify(products)); localStorage.setItem('cart', JSON.stringify(cart)); localStorage.setItem('history', JSON.stringify(history)); }
function openModal() { document.getElementById('add-modal').classList.remove('hidden'); document.getElementById('emoji-search-input').value = ''; filterEmojis(); }
function closeModal() { document.getElementById('add-modal').classList.add('hidden'); document.getElementById('new-prod-name').value = ''; selectedNewEmoji = '🛒'; }
function selectNewEmoji(btn, emoji) { document.querySelectorAll('.emoji-opt').forEach(b => { b.classList.remove('bg-pink-200', 'border-pink-300'); b.classList.add('border-transparent'); }); btn.classList.add('bg-pink-200', 'border-pink-300'); btn.classList.remove('border-transparent'); selectedNewEmoji = emoji; }

function filterEmojis() { 
    const query = document.getElementById('emoji-search-input').value.toLowerCase(); 
    const grid = document.getElementById('emoji-grid'); 
    
    const normalize = str => str.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); 
    const normalizedQuery = normalize(query); 

    let filtered = emojiDatabase.filter(e => normalize(e.tags).includes(normalizedQuery)); 
...
function saveNewProduct() { const name = document.getElementById('new-prod-name').value; const cat = document.getElementById('new-prod-cat').value; if (!name) return alert('Dê um nome ao produto!'); const newId = Date.now(); products.push({ id: newId, name: name, cat: cat, icon: selectedNewEmoji }); saveData(); closeModal(); renderProductGrid(); alert('Produto adicionado! 🎉'); }
