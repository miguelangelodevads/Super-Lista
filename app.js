...
// --- CORREÇÃO DE TELA BRANCA (Recuperação de Erros) ---
...
try {
    ...
    console.error("Erro ao carregar dados salvos. Resetando para o padrão.", e);
    // Se der erro, limpa tudo para não travar de novo
    localStorage.clear();
...
function updateTotalHeader() { let total = 0; Object.values(cart).forEach(item => { if(item.price) total += item.price * item.qty; }); const el = document.getElementById('header-total'); if(el) el.innerText = total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
...
function updateCartBadge() { const count = Object.keys(cart).length; const badge = document.getElementById('cart-badge'); if(badge) { badge.innerText = count; if (count > 0) badge.classList.remove('hidden'); else badge.classList.add('hidden'); } }
function saveData() { 
    try {
        ...
        console.error("Erro ao salvar dados (memória cheia?)", e);
    }
}
...
try {
    ...
    alert("Ocorreu um erro. Tentando reiniciar...");
    localStorage.clear();
    location.reload();
}
